...
lista <- read_csv("lista.csv", col_names = FALSE, 
  ...
  mutate(Grupo = case_when(Nombre %in% c("Alvarado Subiabre, Guillermo Ignacio","Lara Riquelme, Bárbara Escarlen", "Luco Molina, Macarena Alejandra") ~ sample(1:10, size = 1),
                   Nombre %in% c("Rodríguez Oyarzo, María Teresa","Ramírez Águila, Kamila Andrea","Bañares Mansilla, Angie Javiera") ~ sample(1:10, size = 1)))

Asignados <- lista %>% dplyr::filter(!is.na(Grupo))

NoAsignados <- lista %>% dplyr::filter(is.na(Grupo))

Grupos <- rep(1:10, each = 3)

Grupos <- Grupos[!(Grupos %in% unique(Asignados$Grupo))]
set.seed(2021)

NoAsignados$Grupo <- sample(Grupos, size = 24, replace = F)

DF <- data.frame(Grupo = 1:10, Presentacion = seq(ISOdate(2021,1,18, 10, 0, tz = "America/Santiago"), by = "20 min", length.out = 10))
